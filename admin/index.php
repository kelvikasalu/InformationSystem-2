...
<head>
    ...
    <style>
        ...
        /* Add gradient effect to cards */
        .dashboard-items {
            background: linear-gradient(145deg, var(--cardbackground), #f8f9ff);
        }

        /* Add subtle pattern to main background */
        body {
            background-image: linear-gradient(var(--backgroundcolor) 1px, transparent 1px),
                              linear-gradient(to right, var(--backgroundcolor) 1px, transparent 1px);
        ...
        .dashboard-items:hover {
            background: linear-gradient(145deg, #f8f9ff, var(--cardbackground));
...
<body>
    ...
    $dashboard_stats = [
        'patients' => $database->query("SELECT COUNT(*) as count FROM patient")->fetch_assoc()['count'],
        'doctors' => $database->query("SELECT COUNT(*) as count FROM doctor")->fetch_assoc()['count'],
        'appointments' => $database->query("SELECT COUNT(*) as count FROM appointment WHERE appodate >= '$today'")->fetch_assoc()['count'],
        'sessions' => $database->query("SELECT COUNT(*) as count FROM schedule WHERE scheduledate = '$today'")->fetch_assoc()['count']
    ];
    ...
    function generateReport($database, $reportType, $startDate, $endDate) {
        // ... (keep the entire function as provided in the previous response)
    }
    ...
    if (isset($_POST['generate_report'])) {
        ...
        generateReport($database, $reportType, $startDate, $endDate);
    }
                                    ...
                                    <div  class="dashboard-items"  style="padding:20px;margin:auto;width:95%;display: flex">
                                        <div>
                                                <div class="h1-dashboard">
                                                    <?php echo $dashboard_stats['doctors']; ?>
                                                </div><br>
                                                            ...
                                                            while ($row = $result->fetch_assoc()) {
                                                                echo '<tr>
                                                                    <td style="text-align:center;font-size:23px;font-weight:500; color: var(--btnnicetext);padding:20px;">
                                                                        '.htmlspecialchars($row["apponum"]).'
                                                                    </td>
                                                                    <td style="font-weight:600;"> &nbsp;'.
                                                                        htmlspecialchars(truncateText($row["pname"], 25))
                                                                    .'</td>
                                                                    <td style="font-weight:600;"> &nbsp;'.
                                                                        htmlspecialchars(truncateText($row["docname"], 25))
                                                                    .'</td>
                                                                    <td>'.
                                                                        htmlspecialchars(truncateText($row["title"], 15))
                                                                    .'</td>
                                                                </tr>';
                                                            }
                                                        }
                        ...
                        <table width="100%" border="0" class="dashbord-tables">
                            <tr>
                                <td>
                                    ...
                                    <p style="padding-bottom:19px;padding-left:50px;font-size:15px;font-weight:500;color:#212529e3;line-height: 20px;">
                                        Download detailed reports for Appointments, Doctors, and Patients
                                    </p>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <center>
                                        <div class="report-container" style="width:95%;background:white;border-radius:8px;padding:20px;box-shadow:0 0 15px rgba(0,0,0,0.1);">
                                            <!-- Report Generation Form -->
                                            <form method="POST" action="process_report.php" target="_blank">
                                                <div style="display:flex;gap:20px;align-items:center;justify-content:center;margin-bottom:20px;">
                                                    <div>
                                                        <select name="report_type" class="input-text" 
    ...
    <!-- Add this JavaScript for date input toggle -->
    <script>
    document.querySelector('select[name="report_type"]').addEventListener('change', function() {
        ...
        dateInputs.style.display = this.value === 'appointments' ? 'block' : 'none';
    });
    </script>
